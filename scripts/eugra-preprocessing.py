...
dfTypeR = pd.read_csv("../data/rtp_project_data/"+org+"V2.protein.actions.v11.0.txt",
                      sep="\t",
                      engine='python',
                     # compression="gzip",
...
for c in dfSf.columns:
    if "Source" in c or "Target" in c  or "Type" in c or "score" in c  or ("rank" not in c  and "NC_Score" not in c and "D" not in c and "SDev" not in c):
...
print("Done.")
result.to_csv("ToServerUPPMAX/results/"+org+".processed_data.tsv", sep='\t',index=False)
